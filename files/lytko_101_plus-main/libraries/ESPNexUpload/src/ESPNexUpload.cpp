...
//	#ifndef NEXT_RX
		#define NEXT_RX 5	// Nextion RX pin | Default 14 / D5
		#define NEXT_TX 4	// Nextion TX pin | Default 12 / D6
//	#endif
//	#ifndef nexSerial
...
//	#endif
	...
	#ifndef NEXT_RX
		#define NEXT_RX 5	// Nextion RX pin | Default 16
		#define NEXT_TX 4	// Nextion TX pin | Default 17
	#endif
	#ifndef nexSerial
...
bool ESPNexUpload::_searchBaudrate(uint32_t baudrate){
	...
	this->recvRetString(response,500,0);
	if(response[0] != 0x1A){
...
bool ESPNexUpload::setComponentValue(String component, int value){
...
bool ESPNexUpload::setComponentText(String component, String txt){
  String componentText = component + ".txt=\"" + txt + "\"";//Set Component text
  this->sendCommand(componentText.c_str());
  return true;//this->ack();
}//end set_component_txt

uint16_t ESPNexUpload::getComponentText(String component)
{
    String componentText = "get "+component + ".txt";//Get Component text
	this->sendCommand(componentText.c_str());
    return true;
}

bool ESPNexUpload::init(const char* pageId, bool first){
//dbSerialPrintln(F("Serial tests & connect"));
if(first)  nexSerialBegin(9600);//this->_searchBaudrate(9600);
  String pageValue = "page " + String(pageId);//Page
  this->sendCommand(pageValue.c_str());
  return true;
}//end nextion_init

uint16_t ESPNexUpload::available(){
    return nexSerial.available();
}

uint16_t ESPNexUpload::read(){
    return nexSerial.read();
}
uint16_t ESPNexUpload::recvRetString(String &response, uint32_t timeout, uint8_t nr_of_FF_bytes, bool recv_flag){
...
bool ESPNexUpload::_setPrepareForFirmwareUpdate(uint32_t upload_baudrate){
	...
	this->recvRetString(response, 800,0,true); // normal response time is 400ms
...
bool ESPNexUpload::upload(const uint8_t *file_buf, size_t buf_size){
    ...
    for(uint16_t i = 0; i < buf_size; i++){
		...
		if(_sent_packets == 4096){
			...
			this->recvRetString(string,500,0,true);  
...
bool ESPNexUpload::_echoTest(String input) {
	...
	this->recvRetString(response,duration_ms,0); 
