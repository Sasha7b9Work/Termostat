...
int homekit_storage_init() {
    ...
    else ERROR("OK to read HomeKit storage magic");
...
/*        if (!spiflash_write(STORAGE_BASE_ADDR, (byte *)blank, 1408)) {
            ERROR("Failed to erase HomeKit storage");
            return -1;
        }
        else ERROR("OK to erase HomeKit storage");
        ...
        else ERROR("OK to write HomeKit storage magic");
*/
        return 1;
    }
...
/*    if (!spiflash_write(MAGIC_ADDR, blank, sizeof(blank))) {
    ...
    else ERROR("OK to reset HomeKit storage");*/
